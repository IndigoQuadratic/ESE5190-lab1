...
class WorldTraj(object):
    ...
    def Time_segment(self):
        ...
        self.Ti = 1.4 * np.sqrt(self.Ti)  # Operate on the entire array, not on individual elements
        # for i in range(0, self.Ti.shape[0] - 1):
        #     if 0 < self.angles[i] <= 30:
        #         self.Ti[i+1] *= 1.2
        #     elif 30 < self.angles[i] <= 90:
        #         self.Ti[i+1] *= 1.08
        #     elif 90 < self.angles[i] <= 150:
        #         self.Ti[i+1] *= 1.06
        #     elif 150 < self.angles[i] <= 180:
        #         self.Ti[i+1] *= 0.88
